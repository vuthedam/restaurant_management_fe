    ...
    <header className="bg-white border-b sticky top-0 z-30 shadow-sm">
        ...
        <div className="flex items-center gap-4">
          {actionLabel ? (
            <button
              type="button"
              onClick={onAction}
              className="bg-orange-600 text-white px-5 py-2 rounded-lg font-semibold hover:bg-orange-700 transition cursor-pointer"
            >
              {actionLabel}
            </button>
          ) : null}
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className="relative p-2 text-gray-600 hover:text-orange-600 hover:bg-gray-100 rounded-full transition cursor-pointer flex items-center justify-center"
            >
            ...
            {/* Dropdown Menu */}
            {isOpen && (
              <div className="absolute right-0 mt-3 w-80 bg-white/95 backdrop-blur-md border border-gray-100 rounded-2xl shadow-2xl z-50 overflow-hidden animate-scale-up">
                <div className="p-4 border-b bg-gray-50/50 flex justify-between items-center">
                  <h3 className="font-bold text-gray-900 font-display flex items-center gap-1.5">
                    <span className="material-symbols-outlined text-orange-600 text-lg">support_agent</span>
                    Yêu cầu cần hỗ trợ
                  </h3>
                ...
                <div className="max-h-96 overflow-y-auto divide-y divide-gray-100">
                  {activeCalls.length === 0 ? (
                    ...
                    activeCalls.map((call) => {
                      ...
                      return (
                        <div key={call._id} className="p-4 hover:bg-gray-50/50 transition-colors space-y-3">
                          <div className="flex justify-between items-start gap-2">
                            <div className="flex gap-2">
                              ...
                              <div>
                                <span className="font-bold text-xs text-orange-700 bg-orange-50 px-2 py-0.5 rounded border border-orange-100">
                          ...
                          {call.status === "handling" && call.handledBy && (
                            <p className="text-[10px] text-gray-600 bg-gray-50 p-1.5 rounded border border-gray-100">
                          ...
                          <div className="flex gap-2">
                            {call.status === "pending" ? (
                              <button
                                ...
                                className="w-full py-1.5 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-xs font-bold transition cursor-pointer"
                              >
                                Nhận xử lý
                              </button>
                            ) : (
                              <button
                                ...
                                className="w-full py-1.5 bg-green-600 hover:bg-green-700 text-white rounded-lg text-xs font-bold transition cursor-pointer"
                              >
                                Hoàn thành
                              </button>
                            )}
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-3 pointer-events-none">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className="pointer-events-auto bg-orange-600 border border-orange-500 text-white px-5 py-4 rounded-2xl shadow-2xl flex items-center justify-between gap-4 max-w-sm w-80 animate-slide-in"
          >
